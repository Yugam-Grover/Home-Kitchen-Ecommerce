...
| EU | Digital Services Act | Transparent recommendation algorithms |
...
| **Inventory Model** | DTC items shipped from owned/leased warehouses |
...
| Attribute | Nesting Nina (Core Customer) | Comparison Maven (Savvy Shopper) |
|---|---|---|
| **Archetype** | Wellness-conscious homemaker | Variety-seeking value optimizer |
| **Age Range** | 28–42 | 25–50 |
| **Primary Goal** | Safe, non-toxic, curated products for the home | Maximum selection with minimum decision fatigue |
| **Pain Points** | Distrust of ingredient lists; overwhelmed by Amazon's chaos | Too many tabs; inconsistent product quality |
| **Brand Affinity** | Values certifications (Organic, BPA-Free, Fair Trade) | Values reviews, ratings, comparison features |
| **Geographic Reach** | Primarily Tier 1 markets; multi-currency aware | Global; shops across Tier 1 + Tier 2 markets |
| **Device Split** | 65% mobile / 35% desktop | 50% mobile / 50% desktop |
| **Session Behavior** | Browse → Save → Return → Purchase (multi-session) | Search → Compare → Purchase (single-session intent) |

### 2.2 Persona: Nesting Nina (Core Customer)
...
### 2.3 Persona: Comparison Maven

**Scenario:** Maven wants to compare bamboo cutting boards across the catalog without opening 15 tabs.
...
| Purchase | "One seamless checkout" | Unified cart; shipping cost shown before checkout; prices in local currency |
| Post-Purchase | "Track all my items in one place" | Aggregated order tracking dashboard |
...
| 7 | Enters shipping address | Shipping options appear | Address validation via API → calculate shipping → display shipping cost; Gold members see expedited options first |
...
| 9 | Clicks "Place Order" | Order confirmation | `POST /api/orders` → Stripe `PaymentIntent.confirm()` in local currency → on success: decrement inventory (atomic DB transaction) → send confirmation email → redirect to confirmation page |
...
| Real-Time Inventory | PDP — variant selector | WebSocket feed updates stock status live; display "In Stock", "Low Stock (< 5)", or "Out of Stock" badges |
| Estimated Delivery | PDP + Cart + Checkout | Calculate from warehouse SLA (24h) + `shipping_carrier.eta`; Gold members see faster delivery estimate; display date range, not vague "3-5 days" |
...
### 4.4 Inventory & Fulfillment

| Req ID | Requirement | System Behavior | Acceptance Criteria |
|---|---|---|---|
| INV-001 | 24-hour dispatch SLA for warehouse items | Pick/pack/ship within 24h of order placement; priority dispatch for Gold members | Gold orders flagged `priority = true` in warehouse management system |
| INV-002 | Warehouse inventory sync | Real-time sync between warehouse management system (WMS) and platform inventory DB | Stock discrepancy > 5 units triggers reconciliation alert |
...
| **Delivery Speed** | Standard shipping (carrier default ETA) | Priority dispatch (same-day pick) + free expedited shipping |
...
| GOLD-003 | Priority delivery flagging (Gold) | On order placement: if `user.membership_tier === 'gold'` → set `priority = true` on order | Warehouse picks Gold orders first |
| GOLD-004 | Free shipping calculation (Gold) | If Gold: set `shipping_cost = 0` for all items regardless of weight or destination | Shipping cost absorbed by platform (factored into Gold subscription margin) |
...
| Chargeback received | Stripe sends `charge.dispute.created` webhook | Auto-pause fulfillment for disputed item; gather evidence (order details, tracking, delivery confirmation) | Submit dispute evidence via Stripe API within 7 days; if lost: absorb cost |

### 5.4 Membership Edge Cases
...
| Inventory System | WebSocket real-time stock; warehouse inventory sync | INV-001 through INV-002 passing; race condition tests green |
...
| Order Management | Order creation, status tracking, refund flows | Order model working; partial refund tested |
...
│                     │ ✓ Order management    │ ✓ Analytics live      │
...
| ASM-P05 | No dynamic/surge pricing | Mid-market trust: prices are stable; no algorithmic price manipulation | Real-time pricing engine not required; prices change via admin only |

### 8.2 Shipping Assumptions

| Assumption ID | Assumption | Basis | Dependent Requirements |
|---|---|---|---|
| ASM-S01 | DTC warehouse dispatch SLA: 24h | Industry standard for owned fulfillment (2026) | INV-001; delivery ETA calculations |
| ASM-S02 | Average domestic shipping: $5.99–$8.99 | Carrier rate contracts (UPS/FedEx/USPS, 2026 rates); weight-based for mid-market products | Shipping cost calculations; Gold free-shipping subsidy budget |
...
| ASM-I02 | Average SKU count at launch: 2,000–5,000 | Curated catalog; grows to 10,000+ by end of Year 1 | Search index sizing; filter performance; DB schema planning |
| ASM-I03 | Overstock threshold: > 120 days of inventory | Items with > 120 days supply flagged for markdown or bundling | Clearance pricing logic; STK-005 (no loyalty discount on clearance) |
| ASM-I04 | Stockout tolerance: < 2% of active SKUs | Target: 98%+ in-stock rate across catalog | Inventory alert thresholds; reorder point automation |
...
| ASM-T02 | Stripe as sole payment processor | Multi-currency support; PCI compliance offloaded | CHK-001 through CHK-015; INTL-005 |
...
| SEC-004 | Row-Level Security (RLS) | Supabase RLS policies enforce data isolation: users can only access own data | Supabase best practices; GDPR data minimization |

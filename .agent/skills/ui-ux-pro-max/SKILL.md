---
name: ui-ux-pro-max
description: "MANUAL INVOKE ONLY — Do NOT auto-trigger this skill. Only use when the user explicitly requests a 'UX audit' or says 'use the ui-ux-pro-max skill'. Provides UX craft quality auditing: accessibility, interaction patterns, contrast, layout, and anti-pattern detection via a searchable database."
---

# ui-ux-pro-max

Comprehensive UX quality auditing tool. Contains 67 styles, 96 color palettes, 57 font pairings, 99 UX guidelines, and 25 chart types across 13 technology stacks. Searchable database with priority-based recommendations.

## ⚠️ Project-Specific Audit Scope

This project already has a locked design system (`docs/design-system.md`). When auditing:

**✅ USE these domains (UX craft quality):**
- `ux` — Accessibility, animation, z-index, loading states, anti-patterns
- `web` — ARIA, focus, keyboard nav, semantic HTML
- Pre-Delivery Checklist (bottom of this file)

**❌ DO NOT USE these domains (brand is already locked):**
- `color` — Colors are defined in `design-system.md §1.1`
- `typography` — Fonts are Lora/Satoshi/Geist Mono, locked
- `style` — Style is Japandi minimalist, locked
- `product` — Product type patterns already in PRD
- `--design-system` flag — Design system already exists, do not generate a new one
